...
ToolChain::ToolChain(std::string configfile) : m_toolbox()
{
    ...
    if (m_pub_sec < 0 || Inline > 0 || interactive)
        sendflag = false;
    if (m_kick_sec < 0)
        receiveflag = false;
    ...
    if (m_log_mode == "Remote")
        sleep(10); //needed to allow service discovery find time
    logmessage << "UUID = " << m_UUID << std::endl
               << "********************************************************" << std::endl
               << "**** Tool chain created ****" << std::endl
               << "********************************************************" << std::endl;
...
int ToolChain::Initialise()
{
    ...
    {
        logmessage << "********************************************************" << std::endl
                   << "**** Initialising tools in toolchain ****" << std::endl
                   << "********************************************************" << std::endl;
        m_data.Log->Log(logmessage.str(), 1, m_verbose);
        logmessage.str("");
      ...
      *(m_data.Log)<<"********************************************************"<<std::endl<<std::endl;
        ...
        logmessage << std::endl
                   << "**** Tool chain initialised ****" << std::endl
                   << "********************************************************" << std::endl;
        m_data.Log->Log(logmessage.str(), 1, m_verbose);
        logmessage.str("");
...
o execute (uncaught error)" << std::endl;
      ...
      */
            logmessage << "**** Tool chain executed ****" << std::endl
                       << "********************************************************" << std::endl;
        ...
        {
            logmessage << "********************************************************" << std::endl
                       << "**** Executed toolchain " << repeates << " times ****" << std::endl
                       << "********************************************************" << std::endl;
            m_data.Log->Log(logmessage.str(), 2, m_verbose);
            logmessage.str("");
        }
    ...
    */
        ...
        logmessage << "********************************************************" << std::endl
                   << std::endl
                   << " ERROR: ToolChain Cannot Be Executed As Has Not Been Initialised yet." << std::endl
                   << "********************************************************" << std::endl;
        m_data.Log->Log(logmessage.str(), 0, m_verbose);
        logmessage.str("");
        result = -1;
...
int ToolChain::Finalise()
{
    ...
    */
        logmessage << "********************************************************" << std::endl
                   << "**** Finalising tools in toolchain ****" << std::endl
                   << "********************************************************" << std::endl;
        m_data.Log->Log(logmessage.str(), 1, m_verbose);
        logmessage.str("");
    ...
    */
        logmessage << "**** Toolchain Finalised ****" << std::endl
                   << "********************************************************" << std::endl;
        m_data.Log->Log(logmessage.str(), 1, m_verbose);
        logmessage.str("");
    ...
    */
        ...
        logmessage << "********************************************************" << std::endl
                   << std::endl
                   << " ERROR: ToolChain Cannot Be Finalised As Has Not Been Initialised yet." << std::endl
                   << "********************************************************" << std::endl;
        m_data.Log->Log(logmessage.str(), 0, m_verbose);
        logmessage.str("");
...
std::string ToolChain::ExecuteCommand(std::string command)
{
    ...
    if (command != "")
    {
        ...
        else if (command == "Restart")
        {
            ...
            if (ret == 0)
                returnmsg << "Restarting ToolChain";
            else
                returnmsg << "Error Code " << ret;
        }
        ...
        else if (command == "Status")
        {
            ...
            if (Finalised)
                tmp << "Waiting to Initialise ToolChain";
            if (Initialised && execounter == 0)
                tmp << "Initialised waiting to Execute ToolChain";
            if (Initialised && execounter > 0)
            {
                if (paused)
                    tmp << "ToolChain execution pasued";
                else
                    tmp << "ToolChain running (loop counter=" << execounter << ")";
            }

            if (*(m_data.vars["Status"]) != "")
                tmp << " : " << *(m_data.vars["Status"]);
            returnmsg << tmp.str();
        }
        else if (command == "?")
            returnmsg << " Available commands: Initialise, Execute, Finalise, Start, Stop, Restart, Pause, Unpause, Quit, Status, ?";
        else if (command != "")
        {
...
void ToolChain::Remote()
{
    ...
    while (running)
    {
        ...
        if (Ireceiver.recv(&message, ZMQ_NOBLOCK))
        {
            ...
            if (*rr["msg_type"] == "Command")
                command = *rr["msg_value"];
            else
                command = "NULL";
